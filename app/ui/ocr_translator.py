...
from ..core.ocr import find_tesseract, pixmap_to_png
...
class OCRTranslator(QObject):
    ...
    def _on_hotkey(self) -> None:
        ...
        # 2. Tesseract 缺失时托盘引导，不弹覆盖层
        if find_tesseract() is None:
            self.hotkey_status.emit(
                False,
                "未找到 Tesseract，请安装（winget install "
                "UB-Mannheim.TesseractOCR）后重试",
            )
            return
    ...
    def _on_region(self, region: QRect) -> None:
        ...
        languages = self._config.get("ocr", {}).get("languages", "chi_sim+eng")
    ...
    def _cancel_workers(self) -> None:
        ...
        先断信号再等待：OCR 子进程不可中断（最长 10s 自然超时），
