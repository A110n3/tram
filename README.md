...
静态画面几乎零开销；字幕来回闪（A→B→A）不会重复翻译；字幕切换快于翻译速度时新字幕
排队依次翻译，队列满才丢最旧的等待句（快对话不整段漏翻）；鼠标位于监控区域内时
暂停识别，避免悬停引发的画面变化误触发翻译；关闭监控小窗即停止监控。
监控间隔、帧差阈值、相似度阈值、防抖周期、排队条数等可在设置中调整。
...
└── ui/
    ...
    ├── monitor_translator.py   # 区域实时监控编排器（框选 → 监控 → 有界队列翻译）
